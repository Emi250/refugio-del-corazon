...
- [x] **Auditoría UX + curaduría de fotos** — 2026-09-20
  ...
  - Lightbox: precarga de las fotos vecinas, variante de 1100px en mobile (−66% de peso: 1301 KB → 447 KB por unidad), spinner de carga, `history.pushState` (el botón Atrás de Android cerraba la web en vez del visor), focus trap + `inert` en el fondo (los 3 botones eran tabulables con el visor cerrado en TODAS las páginas), contexto de unidad en el contador ("Departamento #3 · 4 / 8"), CTA de WhatsApp adentro, `alt` de las fotos ocultas, ctrl/cmd+click respetado, botones a 44px
...
- [x] **Auditoría SEO técnica** (informe en `docs/seo/AUDIT.md`) — 2026-09-26, desplegada en producción (PR #6) y verificada en el sitio live. Detalle de unidad: se sacó el preload del hero (pedía una variante que no estaba en el `srcset`, 101–263 KB desperdiciados) y la foto 1 del mosaico pasó a `eager` + `fetchpriority=high` (prop `priority` de `PhotoMosaic`). `absoluteUrl()` preserva `#`/`?` (el breadcrumb generaba `/#unidades/`). `UnitSchema` sin `petsAllowed` (contradecía la FAQ) y con imágenes deduplicadas. Geo meta desde `BUSINESS`. Nuevo `npm run seo:check` (`scripts/seo-check.mjs`, sin deps) — correrlo después de cada build. **Ojo: el build real está en `.vercel/output/static/`, no en `dist/`**
- [x] **H1 del home con bajada keyword**: `<span class="hero-deck">` dentro del `<h1>` del `Hero` (clave i18n `hero.deck`), mucho más chica que el nombre (20–30px desktop / 18px mobile) — 2026-09-26
- [x] **Perf mobile (PR #8)**: logo `BrandMark` vía `astro:assets` (72 KB → ~1 KB) + `build.inlineStylesheets: 'always'`. PSI mobile `/unidades/unidad-2/`: rendimiento 72 → 89, LCP 4,9 → 2,9 s; aparece un CLS de 0,059 por el cambio de fuente. Preview del build estático: config `refugio-build` en `.claude/launch.json` — 2026-09-26
- [ ] **Fuentes propias** (`@fontsource-variable/*` + preload) para sacar los 750 ms de Google Fonts y el CLS del cambio de fuente — requiere OK para la dependencia nueva
